...
from fusion_net import FusionNet_MM_KL
import pickle
...
def net_test(net, data_loader, scene_to_event, cal_map=False):
    num = len(data_loader.dataset)
    correct = 0
    net.eval()
    predict_labels = np.array([])
    ground_labels = np.array([])
    testing_scene_to_event = np.zeros((13,527))
    testing_counting = np.zeros((13,))
    with torch.no_grad():
        for i, data in enumerate(data_loader, 0):
            ...
            output, predict_event = net(img, aud)


            event_output_ = predict_event.cpu().numpy()
            for idx in range(event_output_.shape[0]):
                testing_scene_to_event[label[idx],:] += event_output_[idx,:]
                testing_counting[label[idx]] += 1
    ...
    testing_counting = np.tile(np.expand_dims(testing_counting,1),[1,527])
    testing_scene_to_event = testing_scene_to_event / (testing_counting+1e-5)
    err = np.sum(np.abs(testing_scene_to_event-scene_to_event))
    results = sklearn.metrics.classification_report(ground_labels, predict_labels, digits=4)
    (precision, recall, fscore, sup) = sklearn.metrics.precision_recall_fscore_support(ground_labels, predict_labels,
                                                                                       average='weighted')
    acc = correct / num

    return (acc, precision, recall, fscore, results, err, testing_scene_to_event)
...
def main():
    ...
    # all stand up
    fusion_net = FusionNet_MM_KL(image_net, audio_net, num_classes=13)
    ...
    optimizer = optim.Adam(params=fusion_net_cuda.parameters(), lr=args.learning_rate, betas=(0.9, 0.999),
                           weight_decay=0.0001)
    ...
    for e in range(args.epoch):
        ...
        for i, data in enumerate(train_dataloader, 0):
            ...
            # aud = torch.zeros_like(aud)
            img, aud, scene_label, event_label = img.type(torch.FloatTensor).cuda(), aud.type(
        ...
        (val_acc, val_precision, val_recall, val_fscore, results, err, mat) = net_test(fusion_net_cuda, val_dataloader, scene_to_event)
        print(
            'epoch:%d scene loss:%.4f event loss:%.4f val acc:%.4f val_precision:%.4f val_recall:%.4f val_fscore:%.4f ' % (
            e, scene_loss.cpu(), event_loss.cpu(), val_acc, val_precision, val_recall, val_fscore))
        if val_fscore > max_fscore:
            count = 0
            max_fscore = val_fscore
            (test_acc, test_precision, test_recall, test_fscore, results, err, mat) = net_test(fusion_net_cuda, test_dataloader, scene_to_event)
            ...
            print('mark...')
            print('err %.4f' % err)
            np.save('scene_to_event/KL_MM_%d_epoch.npy' % e, mat)
            #print(results)
        ...
        if count == 5:
            ...
            print('test acc:%.4f (%.4f) precision:%.4f (%.4f) recall:%.4f (%.4f) fscore:%.4f(%.4f)' % (
            test_acc_mean, test_acc_std, test_precision_mean, test_precision_std, test_recall_mean, test_recall_std,
